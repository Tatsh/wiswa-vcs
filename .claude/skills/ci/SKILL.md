...
If any changed files are under `wiswa_vcs/` or `tests/`, run the following
agents **in order**:
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `wiswa_vcs/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `wiswa_vcs/commands/main.py`,
...
For Python files, strip the `wiswa_vcs/` prefix and replace `/` with `.` (like module imports).

- Python file `wiswa_vcs/media.py` → `media:`.
- Multiple files under `wiswa_vcs/commands/` → `commands:`.
- Single command file `wiswa_vcs/commands/admin.py` → `commands.admin:`.
