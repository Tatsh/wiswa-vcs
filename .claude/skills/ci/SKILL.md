...
If any changed files are under `wiswa/vcs/` or `tests/`, run the following
...
1. **click-auditor** - validate Click command consistency. **Only run if the project directly uses
   Click and files under `wiswa/vcs/commands/` changed.**
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `wiswa/vcs/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `wiswa/vcs/commands/main.py`,
...
For Python files, strip the `wiswa/vcs/` prefix and replace `/` with `.` (like module imports).

- Python file `wiswa/vcs/media.py` → `media:`.
- Multiple files under `wiswa/vcs/commands/` → `commands:`.
- Single command file `wiswa/vcs/commands/admin.py` → `commands.admin:`.
